...
ArrayList<Integer> weighted = new ArrayList<>();
boolean hasInfected = false;

int chooseNum() {
  if (weighted.isEmpty()) {
    for (int x = 1; x < dist.size()+1; x++) {
      int weight = dist.get(str(x));
      for (int j = 0; j < weight; j++) {
        weighted.add(x);
      }
    }
  }
  int i = weighted.get(int(random(weighted.size())));
  return i;
}
...
Display getDisplay() {
  ...
  if (i == 11 && windows.size() < 4) return(getDisplay());
...
class D9 extends Display {
  ...
  void draw() {
    pg.beginDraw();
    rs.stream().forEach(r-> {
      if (in(r)) r.c = color((frameCount/2)%360, random(100), 80);
      pg.fill(r.c);
      pg.noStroke();
  ...
  boolean in(R2 r) {
    ...
    if (x > r.start.x - 10 && x < r.end.x + 10 &&
      y > r.start.y - 10 && y < r.end.y + 10) return true;
    return false;
  }
}
...
class D11 extends Display {
  ...
  void infect() {
    for (int i = 0; i < windows.size(); i++) {
      ...
      if (PVector.dist(w.center, window.center) < 200) {
        if (!w.display.infected) accessWindow(i, "infect");
      }
    }
  }
}
class D12 extends Display {
  //img
